...
//获取数值
unsigned char baccaratLogic::GetCardValue(unsigned char cbCardData)  {
    return cbCardData & LOGIC_MASK_VALUE;
}

//获取花色
unsigned char baccaratLogic::GetCardColor(unsigned char cbCardData)  {
    return cbCardData & LOGIC_MASK_COLOR;
}

// 文字
std::string baccaratLogic::GetCardText(unsigned char cbCardData) {
...
unsigned char baccaratLogic::GetArea(const std::string &area)
{
    if (0 == area.compare("闲")){
        return AREA_XIAN;
         ...
         return AREA_XIAN_TIAN;
    }else if (0 == area.compare("庄天王")){
         return AREA_ZHUANG_TIAN;
    }else if (0 == area.compare("同点平")){
         return AREA_TONG_DUI;
    }else if (0 == area.compare("闲对子")){
         return AREA_XIAN_DUI;
    }else if (0 == area.compare("庄对子")){
         return AREA_ZHUANG_DUI;
    }
    return 0xFF;
}
