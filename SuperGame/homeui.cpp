...
void HomeUI::updateGameList(go::GameList &list)
{
    ...
    for(int i = 0; i < list.items_size(); i++)
    {
        ...
        // 添加游戏单元 到 沙盒
        GameButton* gameItemBtn = new GameButton(GameName,ui->scrollAreaWidgetContents);
        ...
        // 添加控件效果
        QPropertyAnimation* animation = new QPropertyAnimation(gameItemBtn,"alpha");
        animation->setDuration(1000);
        animation->setKeyValueAt(0, 100);
        animation->setKeyValueAt(0.5, 125);
...
void HomeUI::onEnterGame(int gameID)
{
    S_GameID = gameID;
    HandleMsg::GetInstance()->ReqEnterGame(gameID);
}
...
void HomeUI::on_pushButton_clicked()
{
    ...
    for(int i = 0; i < vecNums.size(); i++)
    {
        ...
        if(nullptr != gameBtn)
        {
            ...
            // 闪烁动画
            QPropertyAnimation* animation = new QPropertyAnimation(gameBtn,"alpha");
    ...
    {
        qDebug()<<"点击->"<<selectID;
        QPropertyAnimation* animation = new QPropertyAnimation(selectGameBtn,"alpha");
